...
// @TODO: Scheduler can go, right now it resolves its promise with null because
// that's what it did if a profile didn't specify an interval (i.e. wasn't going
...
SpeedTracker.prototype._saveTest = function (profile, content, isScheduled) {
  ...
  // @TODO: Doing away with scheduled tests, but including the profile name here would be nice
...
/**
 * This looks up the destination repository's config file speedtracker.yml
 * With my edits (TBD what I'm doing with Alerts), almost nothing is being
 * used here...
 */
SpeedTracker.prototype.getConfig = function (force) {
  ...
  return this._getRemoteFile('speedtracker.yml').then(data => {
    try {
      ...
      // @TODO: Not using encrypted key or WPT key from this file, that only
      // leaves Alerts subscription / destination information. What to do with that?
...
SpeedTracker.prototype.initConfig = function () {
  return this.getConfig().then(config => {
    // @TODO: This function used to take getConfig and "decrypt" the encryption
    // key. I'm not using that anymore, so this function is a pass-thru on the
    // config. Can probably eliminate.
    return this.config;
  })
}

SpeedTracker.prototype.initWpt = function (profile) {
  let wptUrl = config.get('wpt.url')
  let wptKey = config.get('wpt.key')
...
// init: This is the first method called from index.js
SpeedTracker.prototype.runTest = function (profile, isScheduled) {
  ...
  return this.initConfig().then(() => {
    ...
    return new Promise((resolve, reject) => {
      return this._runWptTest(url, parameters, wpt => {
        ...
        return this._getPagespeedScore(url).then(score => {
          return this._buildResult({
            ...
            // Save test
            return this._saveTest(profile, result, false)
            // ^^ @TODO: false was isScheduled and is only used to set the commit message in _saveTest

            // @TODO! To keep the promise chain moving, I return _saveTest's
            // promise but what I need to do is make a decision on
            // _processBudgets and figure a way to merge both of those so that
            // the resolution is pending both promise chains.

            // Process budgets
            // this._processBudgets(profileData, result)
            // @TODO: Current test running without budgets so this will auto-resolve, need to confirm it
          }).then(result => { resolve(result) })
        })
      }).catch(err => {
        return Promise.reject(Utils.buildError('WPT_ERROR', err.statusText))
        // @TODO: At this point in the original application it made sense for an
        // error here to probably be WPT. But now this error could be caused by
        // many things.
      })
    })
  })
